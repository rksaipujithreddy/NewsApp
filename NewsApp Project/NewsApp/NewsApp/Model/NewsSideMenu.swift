...
public protocol NewsSideMenuDelegate: class {
...
public protocol NewsSideMenuProtocol: class {
    var sideMenu : NewsSideMenu? { get }
    func setContentViewController(_ contentViewController: UIViewController)
}

public enum NewsSideMenuAnimation : Int {
    case none
    case `default`
}
/**
...
*/
public enum NewsSideMenuPosition : Int {
    case left
    case right
}
...
public extension UIViewController {
    ...
    */
    ...
    func fixSideMenuSize() {
        if let navController = navigationController as? NewsSideMenuNavigationController {
    ...
    :returns: A `UIViewController`responding to `NewsSideMenuProtocol` protocol
    */
    func sideMenuController () -> NewsSideMenuProtocol? {
        var iteration : UIViewController? = parent
        if (iteration == nil) {
            return topMostController()
        }
        repeat {
            if (iteration is NewsSideMenuProtocol) {
                return iteration as? NewsSideMenuProtocol
    ...
    internal func topMostController () -> NewsSideMenuProtocol? {
        ...
        var lastMenuProtocol : NewsSideMenuProtocol?
        while (topController?.presentedViewController != nil) {
            if(topController?.presentedViewController is NewsSideMenuProtocol) {
                lastMenuProtocol = topController?.presentedViewController as? NewsSideMenuProtocol
            }
        ...
        if (lastMenuProtocol != nil) {
            ...
            return topController as? NewsSideMenuProtocol
        }
    }
}

open class NewsSideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    fileprivate var menuPosition:NewsSideMenuPosition = .left
    ...
    open weak var delegate : NewsSideMenuDelegate?
    ...
    /**
    Initializes an instance of a `NewsSideMenu` object.

    :param: sourceView   The parent view of the side menu view.
    :param: menuPosition The position of the side menu view.

    :returns: An initialized `NewsSideMenu` object, added to the specified view.
    */
    public init(sourceView: UIView, menuPosition: NewsSideMenuPosition, blurStyle: UIBlurEffect.Style = .light) {
        super.init()
        ...
        self.panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(NewsSideMenu.handlePan(_:)))
        panRecognizer!.delegate = self
        sourceView.addGestureRecognizer(panRecognizer!)

        // Add right swipe gesture recognizer
        let rightSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(NewsSideMenu.handleGesture(_:)))
        rightSwipeGestureRecognizer.delegate = self
        rightSwipeGestureRecognizer.direction =  .right

        // Add left swipe gesture recognizer
        let leftSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(NewsSideMenu.handleGesture(_:)))
    ...
    :returns: An initialized `NewsSideMenu` object, added to the specified view, containing the specified menu view controller.
    */
    public convenience init(sourceView: UIView, menuViewController: UIViewController, menuPosition: NewsSideMenuPosition, blurStyle: UIBlurEffect.Style = .light) {
    ...
    */
    ...
    fileprivate func toggleMenu (_ shouldOpen: Bool) {
        if shouldOpen, delegate?.sideMenuShouldOpNewsSideMenu() == false {
            return
        }
        updateSideMenuApperanceIfNeeded()
    ...
    open func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if delegate?.sideMenuShouldOpNewsSideMenu() == false {
            return false
        }
...
extension NewsSideMenu: UIDynamicAnimatorDelegate {
